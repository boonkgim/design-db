...
The store was chosen in the tech stack document; this is reading its capabilities, not
re-opening the choice.

| Ask | Why it decides things |
|---|---|
| Are there real multi-statement transactions, or only per-statement atomicity? | Decides whether "two tables must agree" is enforceable at all, or has to become one table |
| What is the default isolation level, and is `serializable` available? | Decides whether a read-then-write needs an explicit lock, a guarded single statement, or a retry loop |
...
expressible without transactions, or the data model has found a genuine gap and it goes in
section 11 as a note back to the stack document.
