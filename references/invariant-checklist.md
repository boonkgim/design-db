...
| Which extensions can be installed on the managed plan?                          | Exclusion constraints need `btree_gist`; a managed provider may not allow it. Check it against the plan before designing on it, and print the source and date                                                            |
| Is there a hard row, size, or connection limit?                                 | From the hosting plan. If connections are pooled by whatever sits between the app and the database, say so rather than budgeting for direct ones                                                                          |
...
| Does this row belong to a person?                                                                                            | If an auth library already generates a `user` table, this row references it and inherits its key type. That is not this document's decision to make, and the mismatch with the key strategy is stated in section 3 rather than resolved by re-keying a generated file |
...
| Can one user ever act on another's rows — an operator, a second person on the account, support? | A grant table joining user to account with a role, and an API that takes the account as an argument and checks it. Reading the account off the session instead forecloses this in every resolver at once |
| Can access be granted and revoked, and does anything survive revocation? | Rows in that grant table, inserted and updated. Anything copied out of a grant onto the rows it governs outlives the revocation |
